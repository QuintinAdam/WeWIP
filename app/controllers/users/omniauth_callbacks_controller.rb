class Users::OmniauthCallbacksController < Devise::OmniauthCallbacksController
  Devise.omniauth_configs.keys.each do |provider|
    define_method provider do
      redirect_to root_path, alert: "Somthing went wrong." if auth.nil?
  ...
  def create_user
    ...
    user.user_connected_accounts.new(connected_account_params)

    user.save!

    run_connected_callback(user.user_connected_accounts.last)
  ...
  def attach_account
    connected_account = current_user.user_connected_accounts.create(connected_account_params)
    run_connected_callback(connected_account)
  ...
  def connected_account
    @connected_account ||= UserConnectedAccount.for_auth(auth)
  end
